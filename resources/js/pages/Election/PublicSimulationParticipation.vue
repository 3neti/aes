...
<template>
    ...
    >
        <section
            class="w-full max-w-2xl border-t-8 border-blue-800 bg-white p-6 shadow-sm sm:p-8"
        >
            ...
            <section class="mt-6 border-y border-stone-200 py-5">
                <ul class="space-y-3 text-sm leading-6 text-stone-800">
                    <li
                        v-for="practice in policy.data_practices"
                        :key="practice"
                    >
                        {{ practice }}
                    </li>
                </ul>
                <p class="mt-5 text-sm font-semibold text-stone-900">
                    Simulation evidence is scheduled for review for
            ...
            >
                <button
                    class="min-h-12 w-full bg-blue-800 px-5 font-bold text-white disabled:opacity-50"
                    type="submit"
                    :disabled="processing"
                >
                    {{
                        processing
                            ? 'Opening voting station...'
