...
- Added a role-paired Multi-Tablet Review Room with a facilitator console, signed join links and QR codes, one-browser station binding, and isolated officer, voter, print-station, watcher, and presentation routes.
- Five voter tablets are created by default and up to ten may be assigned. Unpaired devices see only pairing instructions; only the facilitator or paired officer can inspect station join credentials.
- Added a projection-safe presentation screen showing lifecycle progress, paper-operational counts, station readiness, and the pre-close results seal without individual voter selections.
- Added an append-only, hash-chained room event journal and shared Redis locks around room creation, pairing, and closure.
