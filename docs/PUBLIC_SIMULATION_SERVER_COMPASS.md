...
- `election:browser-walkthrough public-simulation --ballots=1` now creates a fresh public simulation round, records the lobby -> officer handoff monitor -> fixed booth voter tablet -> central print station -> closeout -> watcher -> debrief flow in Chromium, renders the public precinct ballot, tally sheet, Election Return, and A4/thermal print-form PDFs into storyboard pages, and produces `public-simulation.webm`, `walkthrough-storyboard.html`, `walkthrough-storyboard.pdf`, recorder metadata, screenshots, and a verified final evidence archive.
