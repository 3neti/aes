...
function checkpointDefinition(filename, details = {}) {
    if (details.printed_artifact) {
        const artifact = details.printed_artifact;
        const definitions = {
            ballot: {
                operator:
                    'The Electoral Board inspects the ballot exactly as produced by the configured file printer after the voter finalized the private ballot.',
                control:
                    'This page image was rendered from the generated ballot PDF. The storyboard records the original PDF path and SHA-256 without reconstructing its contents.',
                verify: 'Review candidate names and order, contest limits, precinct and ballot identifiers, paper serial, QR placement and legibility, margins, font size, and whether the printout protects ballot secrecy during handling.',
            },
            tally_sheet: {
                operator:
                    'After all accepted paper ballots are counted, the Electoral Board inspects the generated tally sheet.',
                control:
                    'This page image comes directly from the tally sheet PDF generated from accepted one-file-per-ballot records.',
                verify: 'Recompute candidate and contest totals from accepted ballot records. Check precinct identity, accepted ballot count, ordering, page readability, and space needed for official annotations or signatures.',
            },
            election_return: {
                operator:
                    'The Electoral Board inspects the Election Return generated from the completed tally before approval and distribution.',
                control:
                    'This page image comes directly from the Election Return PDF; its source path and SHA-256 bind the visual review to the generated artifact.',
                verify: 'Check precinct and ballot-style identity, every contest and candidate total, totals consistency, approval and signature fields, copy/posting suitability, page numbering, and legibility at normal print scale.',
            },
            transmission_report: {
                operator:
                    'The Electoral Board reviews the official handoff or transmission report prepared after Election Return approval.',
                control:
                    'The rendered page is tied to the original report PDF by path, byte count, and SHA-256.',
                verify: 'Check sending and receiving identifiers, timestamps, package references, status language, hash presentation, and signature or acknowledgment fields.',
            },
            delivery_receipt: {
                operator:
                    'The receiving custodian reviews and acknowledges the delivery receipt for the evidence package.',
                control:
                    'The storyboard renders the actual receipt PDF generated during the handoff ceremony.',
                verify: 'Check precinct and package identity, sender and recipient details, custody time, acknowledgment language, signature space, and practical readability when printed.',
            },
            final_backup_report: {
                operator:
                    'The Electoral Board reviews the record of the final backup made after official handoff preparation.',
                control:
                    'The displayed page is a rendering of the generated final backup report PDF.',
                verify: 'Check media identity, destination, timestamps, referenced manifest or archive hashes, completion status, and officer acknowledgment fields.',
            },
            custody_record: {
                operator:
                    'Custodians inspect the ongoing custody record for election evidence and physical materials.',
                control:
                    'The rendered page is derived from the custody record PDF generated by the appliance.',
                verify: 'Check every custodian, handoff time, item or package identifier, seal and condition fields, signatures, and whether the layout supports an unbroken chain of custody.',
            },
            custody_turnover_report: {
                operator:
                    'The outgoing and receiving custodians inspect the final precinct turnover report.',
                control:
                    'The displayed page comes from the generated custody turnover report PDF and retains its source-document hash.',
                verify: 'Check outgoing and receiving identities, scope of materials, seal and condition statements, timestamps, exceptions, signatures, and print readability.',
            },
        };
        const definition = definitions[artifact.type] ?? {
            operator:
                'The Electoral Board inspects this printable ceremony artifact.',
            control:
                'The storyboard renders the actual generated PDF and records its source-document hash.',
            verify: 'Check identity, completeness, readability, signatures, and suitability for the intended election ceremony.',
        };

        return {
            ceremony: 'Printed Artifacts for Review',
            title: `${artifact.title} - page ${artifact.page} of ${artifact.page_count}`,
            ...definition,
            evidence: [artifact.source_relative_path],
        };
    }
    ...
    const checkpointPages = storyboard.checkpoints
        ...
        <section class="page checkpoint">
            ...
            <div class="checkpoint-layout">
                <a class="screenshot-frame" href="${escapeHtml(checkpoint.screenshot.relative_path)}">
                ...
                <div class="narrative">
                    ...
                    ${checkpoint.source_document ? `<section class="source-document"><h3>Original printable document</h3><p><code>${escapeHtml(checkpoint.source_document.relative_path)}</code><br>Page ${escapeHtml(checkpoint.source_document.page)} of ${escapeHtml(checkpoint.source_document.page_count)} · ${escapeHtml(checkpoint.source_document.bytes)} bytes<br>SHA-256 <code>${escapeHtml(checkpoint.source_document.sha256)}</code></p></section>` : ''}
                    <p class="hash"><strong>${checkpoint.source_document ? 'Rendered page' : 'Full-page screenshot'} SHA-256</strong><br>${escapeHtml(checkpoint.screenshot.sha256)}<br><strong>Storyboard frame SHA-256</strong><br>${escapeHtml(checkpoint.storyboard_frame.sha256)}</p>
                </div>
            </div>
        </section>`,
        )
...
export async function generateWalkthroughStoryboard({
    ...
    for (const [index, action] of screenshotActions.entries()) {
        ...
        checkpoints.push({
            ...
            screenshot: {
                filename,
                relative_path: path.relative(artifactDirectory, screenshotPath),
            ...
            source_document: action.details.printed_artifact
                ? {
                      relative_path:
                          action.details.printed_artifact.source_relative_path,
                      filename: action.details.printed_artifact.source_filename,
                      bytes: action.details.printed_artifact.source_bytes,
                      sha256: action.details.printed_artifact.source_sha256,
                      page: action.details.printed_artifact.page,
                      page_count: action.details.printed_artifact.page_count,
                  }
                : null,
        });
    }
    ...
    const storyboard = {
        ...
        statistics: {
            ...
            printed_artifacts_rendered:
                statistics.printed_artifacts_rendered ?? 0,
            printed_artifact_pages: statistics.printed_artifact_pages ?? 0,
